...
import { abi as IErc20 } from '~~/components/aave/Lend/abis/erc20.json';
import { Address } from "~~/components/scaffold-eth";
import { abi as IStableDebtToken } from '~~/components/aave/Lend/abis/StableDebtToken.json'
...
import { useAaveData } from "~~/components/aave/Lend/AaveData"
import AccountSummary from "~~/components/aave/Lend/AccountSummary"
import AccountSettings from "~~/components/aave/Lend/AccountSettings"
...
function Ape({ selectedProvider }) {
  ...
  const { data: AaveApeContract } = useDeployedContractInfo("AaveApe");
  const { data: AaveApeEvents } = useScaffoldEventHistory({
    contractName: "AaveApe",
    eventName: "Ape",
    fromBlock: 0n,
  });
  ...
  const { reserveTokens, assetData, userAccountData, userConfiguration, userAssetList, userAssetData } = useAaveData({ selectedProvider, signer })
  ...
  const { writeContractAsync } = useScaffoldWriteContract("AaveApe");
  ...
  const getCreditAllowance = async () => {
    if(reserveTokens && borrowAssetData) {
      ...
      let aaveApeAddress = AaveApeContract.address;
  ...
  const setFullCreditAllowance = async () => {
    if(reserveTokens && assetData && borrowAssetData) {
    try {
      setDelegating(true)
  ...
  const getATokenAllowance = async () => {
    if(reserveTokens && assetData && apeAssetData) {
  ...
  usePoller(async () => {
    if(AaveApeContract && AaveApeEvents) {
      let _apeEvents = AaveApeEvents;
  ...
  return (
    ...
    <Card title={<Space>
                    ...
                    (AaveApeContract) ?
                    <Address
                        address={AaveApeContract.address}
                        fontSize={16}
                    /> :
        ...
        <Steps>
          ...
          <Step status={hasDelegatedCredit?'finish':'wait'} title="Go ape!"
            description={creditDelegated&&(hasDelegatedCredit&&<>
              ...
              let _ape = writeContractAsync({
                functionName: "ape",
                args: [
                  apeAssetData.underlyingAsset,
                  borrowAssetData.underlyingAsset,
                  debtLookup[debtType]
                ]
              })
              console.log(_ape)
