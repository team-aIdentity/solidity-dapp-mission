...
function Lend({ selectedProvider, ethPrice }) {
  ...
  const httpProvider = new Web3.providers.HttpProvider(targetNetwork.rpcUrls.default.http[0]);

  let txBuilder
  let lendingPool

  if(httpProvider) {
    let aaveNetwork = targetNetwork.id === 11155111 ? Network.sepolia : Network.main
    txBuilder = new TxBuilderV2(aaveNetwork, httpProvider);
  ...
  const { assetData, userAccountData, userConfiguration,  userAssetList, userAssetData, contracts } = useAaveData({ selectedProvider, signer, liveAsset })
